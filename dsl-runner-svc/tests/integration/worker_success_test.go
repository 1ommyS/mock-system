...
type capturePublisher struct {
	messages []domain.ResultMessage
}

func (p *capturePublisher) Publish(ctx context.Context, msg domain.ResultMessage) error {
...
func TestWorkerSuccess(t *testing.T) {
	...
	inputBytes, err := os.ReadFile("data/job_input.json")
	if err != nil {
	...
	job, err := store.Jobs.GetByID(context.Background(), msg.JobID)
	if err != nil {
	...
	expectedBytes, err := os.ReadFile("data/job_expected.json")
	if err != nil {
	...
	msgOut := pub.messages[0]
	if msgOut.Status != domain.JobStatusDone {
		t.Fatalf("expected DONE event, got %s", msgOut.Status)
	}
	if msgOut.GeneratedCount != 2 {
		t.Fatalf("expected generatedCount 2, got %d", msgOut.GeneratedCount)
	}
	if msgOut.ResultBytes != res.ResultBytes {
		t.Fatalf("expected resultBytes %d, got %d", res.ResultBytes, msgOut.ResultBytes)
	}
}
...
func TestWorkerIdempotency(t *testing.T) {
	...
	if len(pub.messages) < 2 {
		t.Fatalf("expected republished result")
	}
}
